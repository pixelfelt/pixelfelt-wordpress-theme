const App = new Vue({
  ...
  destroyed () {
    handsfree.plugin.displayShape.disable()
    handsfree.plugin.recordShapes.disable()
  },
  ...
  methods: {
    ...
    startDemo (model, callback) {
      handsfree.update(this.demoOpts[model], callback)
    },
    ...
    countdown () {
      if (--countdown > 0) {
        ...
        handsfree.plugin.recordShapes.enable()
    ...
    renderRecording () {
      ...
      this.recordedShapes.forEach((recording, frame) => {
        ...
        $canvas.width = handsfree.debug.$canvas.hands.width
        $canvas.height = handsfree.debug.$canvas.hands.height
    ...
    generateGestureDescription () {
      ...
      handsfree.useGesture(this.gesture)
    },
